...
function EPJS_ShowPreview( e, preview_id ) 
{
	...
	box.style.left = screen_w/2 - box_w/2 + "px";
...
var statsLanguage = document.getElementsByTagName('html')[0].getAttribute('lang');
var statsLocaleLoading;
var statsLocaleCountry;
var statsLocaleDownloads;
var statsLocalePlaceholder;

if ( statsLanguage == 'fr' ) {
// Francais:

        statsLocaleLoading = 'Chargement';
        statsLocaleCountry = 'Pays';
        statsLocaleDownloads = 'T\u00E9l\u00E9chargements';
        statsLocalePlaceholder = 'ex. Tremblay, Martin';


} else {

// English:
        statsLocaleLoading = 'Loading';
        statsLocaleCountry = 'Country';
        statsLocaleDownloads = 'Downloads';
        statsLocalePlaceholder = 'e.g. Smith, John';
...
//		this.run_lock = true;
	...
	wait: function() {
		$( this.container_id ).insert( new Element( 'img', { 'border': '0', 'class': 'irstats2_spin', 'src': '/style/images/loading.gif' } ) );
	     $( this.container_id ).insert( '<span class="irstats2_loading">' + statsLocaleLoading + '...</span>' );
	}
...
var EPJS_Stats_GoogleGeoChart = Class.create(EPJS_Stats, {
	...
	ajax: function($super,response) {
		...
		var data = new google.visualization.DataTable();
		data.addColumn('string', statsLocaleCountry );
		data.addColumn('number', statsLocaleDownloads );
		data.addRows( jsdata );
...
var EPJS_Set_Finder_Autocomplete = function( container_id, select_id, input_id, base_url, context ) {
	...
	var placeholder = statsLocalePlaceholder;
...
/* From: /usr/share/eprints/lib/static/javascript/auto/polypublie.js */

/*   

PolyPublie.js
Fonctions personnalisées pour PolyPublie
2016-2018 robin.sylvestre@polymtl.ca

*/

/*
Remplacer les tirets et normaliser l'affichage des balises Résumé et Abstract.
*/

function cleanAbs() {
	if (document.getElementById('abstrct_text')) {
		document.getElementById('abstrct_text').innerHTML = document.getElementById('abstrct_text').innerHTML.replace(/-{9,20}/g, '<br /><br />'); 
		document.getElementById('abstrct_text').innerHTML = document.getElementById('abstrct_text').innerHTML.replace(/(\br\xE9sum\xE9\s:\s|\br\xE9sum\xE9:\s|\br\xE9sum\xE9\s|\bresume\s:\s|\bresume:|\bresume\s|"r\xE9sum\xE9:")/i, '<span class="abstrct_text_title">R\xC9SUM\xC9 </span>'); 
		document.getElementById('abstrct_text').innerHTML = document.getElementById('abstrct_text').innerHTML.replace(/(\babstract\s:\s|\babstract:\s|\babstract\s|"abstract:")/i, '<span class="abstrct_text_title">ABSTRACT </span>'); 	
	}
}
...
function showAM(elementWatch, elementShow) { 
	if ( document.getElementById( elementWatch ).innerHTML != "" ) { 
...
function toggleEmbargoDate( ) {
	if ( ( currentArticles.value == "TRUE" && currentStatutDiplome.value == "do" ) || ( currentStatutDiplome.value == "other" ))  {
			...
			if ( currentStatutDiplome.value == "other" ) {
				setEmbargoDate( "true" );
			} else {
				setEmbargoDate( "false" );
			}
...
function setEmbargoDate( unset ) {

	if ( originalValues[0] == "" && originalValues[1] == "00" && originalValues[2] == "00" && unset == "false" ) {

		var embargoDate = new Date();
		embargoDate.setFullYear(embargoDate.getFullYear(), embargoDate.getMonth() + 12); // ajoute ce nombre de mois
		var dd = embargoDate.getDate();
		var mm = embargoDate.getMonth()+1; // janvier = 0
		var yyyy = embargoDate.getFullYear();
		...
		embargoDateFields[0].value = yyyy;
		embargoDateFields[1].value = mm;
		embargoDateFields[2].value = dd;
	} 

	if ( unset == "true" ) {

		embargoDateFields[0].value = originalValues[0];
		embargoDateFields[1].value = originalValues[1];
		embargoDateFields[2].value = originalValues[2];
