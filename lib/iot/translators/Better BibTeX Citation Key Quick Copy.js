{
	...
	"inRepository": false,
	"lastUpdated": "2018-05-31 14:51:10"
}
...
Object.defineProperty(exports, "__esModule", { value: true });
const debug_formatter_ts_1 = __webpack_require__(/*! ../../content/debug-formatter.ts */ 2);
function debug(...msg) {
...
Object.defineProperty(exports, "__esModule", { value: true });
const stringify = __webpack_require__(/*! json-stringify-safe */ 1);
...
Object.defineProperty(exports, "__esModule", { value: true });
const jabref_ts_1 = __webpack_require__(/*! ../bibtex/jabref.ts */ 3); // not so nice... BibTeX-specific code
const debug_ts_1 = __webpack_require__(/*! ../lib/debug.ts */ 0);
...
Object.defineProperty(exports, "__esModule", { value: true });
const format = __webpack_require__(/*! string-template */ 5);
const exporter_ts_1 = __webpack_require__(/*! ./lib/exporter.ts */ 4);
const debug_ts_1 = __webpack_require__(/*! ./lib/debug.ts */ 0);
function select_link(item) {
    const m = item.uri.match(/\/(users|groups)\/([0-9]+|(?:local\/[^\/]+))\/items\/([A-Z0-9]{8})$/);
    if (!m)
        throw new Error(`Malformed item uri ${item.uri}`);
    const [, type, groupId, key] = m;
    let id;
    switch (type) {
        case 'users':
            if (groupId.indexOf('local') !== 0)
                debug_ts_1.debug(`Link to synced item ${item.uri}`);
            id = key;
            break;
        case 'groups':
            if (!groupId)
                throw new Error(`Missing groupId in ${item.uri}`);
            id = `${groupId}_${key}`;
            break;
    }
...
; Zotero.debug('BBT: loaded translators/Better BibTeX Citation Key Quick Copy.ts'); } catch ($wrap_loader_catcher_translators_Better_BibTeX_Citation_Key_Quick_Copy_ts) { Zotero.debug('Error: BBT: load of translators/Better BibTeX Citation Key Quick Copy.ts failed:' + $wrap_loader_catcher_translators_Better_BibTeX_Citation_Key_Quick_Copy_ts + '::' + $wrap_loader_catcher_translators_Better_BibTeX_Citation_Key_Quick_Copy_ts.stack) };
