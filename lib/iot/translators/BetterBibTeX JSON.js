{
	...
	"priority": 100,
	"inRepository": false,
	"lastUpdated": "2018-05-31 14:51:11"
}
...
Translator.detectImport = () => {
    let str;
    debug_ts_1.debug('BetterBibTeX JSON.detect: start');
    let json = '';
...
Translator.doImport = () => {
    ...
    for (const source of data.items) {
        ...
        for (const field of Object.keys(source)) {
            if (!validFields[source.itemType][field])
                throw new Error(`unexpected ${source.itemType}.${field}`);
        }
    ...
    for (const collection of collections) {
        ...
        collection.zoteroCollection.children = collection.items.filter(id => {
            if (items.has(id))
                return true;
            debug_ts_1.debug(`Collection ${collection.key} has non-existent item ${id}`);
            return false;
        }).map(id => ({ type: 'item', id }));
    }
    for (const collection of collections) {
        if (collection.parent && data.collections[collection.parent]) {
            ...
            if (collection.parent)
                debug_ts_1.debug(`Collection ${collection.key} has non-existent parent ${collection.parent}`);
            collection.parent = false;
        }
    }
    for (const collection of collections) {
...
Translator.doExport = () => {
    let item;
    debug_ts_1.debug('starting export');
    const data = {
        config: {
    ...
    while ((item = Zotero.nextItem())) {
        ...
        debug_ts_1.debug('adding item', item.itemID);
        data.items.push(item);
    }
    debug_ts_1.debug('data ready');
    Zotero.write(JSON.stringify(data, null, '  '));
    debug_ts_1.debug('export done');
};
; Zotero.debug('BBT: loaded translators/BetterBibTeX JSON.ts'); } catch ($wrap_loader_catcher_translators_BetterBibTeX_JSON_ts) { Zotero.debug('Error: BBT: load of translators/BetterBibTeX JSON.ts failed:' + $wrap_loader_catcher_translators_BetterBibTeX_JSON_ts + '::' + $wrap_loader_catcher_translators_BetterBibTeX_JSON_ts.stack) };
