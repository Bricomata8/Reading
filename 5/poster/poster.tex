...
\headerbox{1. Introduction}{name=introduction,column=0,row=0, span=3}{
    ...
    \textbf{LoraWan} is one of this emerging wireless communication \cite{ayoub_internet_22}, it allows sensors to reach the gatheway in a range of 5Km. Unlike other technologies Lorawan is the best versatile sollution to deploaye IoT apllication in both urban and rural area where there is no communication infrastructure.
}

\headerbox{2. Genetic Algorithm}{name=model,column=0,below=introduction,span=1}{
    A genetic algorithm is a search heuristic that is inspired by Charles Darwin’s theory of natural evolution \cite{vlahogianni_optimized_2005}.
    This algorithm reflects the process of natural selection where the fittest configurations are selected for reproduction in order to produce offspring of the next generation.
        ...
        \item[\ding{224}]  \textbf{Population:} QoS of all configuration.
    \end{itemize}
...
\headerbox{3. Parameters selection}{name=mcs,column=0,below=model,span=1}{
    The physicla layer of Lora thecgnology (Semtech SX1276) hase 4 parameters which make 6720 possible settings \cite{noura_interoperability_2018}:
    \begin{itemize}
...
\headerbox{4. Experimentaion}{name=screen,span=2,column=1,below=introduction}{
...
\setlength{\columnsep}{-2.5cm}
    \begin{multicols}{2}
    \textbf{Define:} stopping criteria, population size P, and mutation probability pm\\
    \textbf{Generate} randomly  an initial population of chromosomes\\
    \textbf{repeat:}\\
    . . . \textbf{for} each chromosome do\\
    . . . . . . Train a model \& compute chromosome's fitness\\
    . . . \textbf{end}\\
    . . . \textbf{for} each reproduction 1 ... P/2 do\\
    . . . . . . \textbf{Select:} 2 chromosomes based on fitness\\
    ...
    \begin{tikzpicture}[node distance=1cm, every node/.style={fill=white, font=\sffamily}, align=center]
      \node (start)       [activityStarts]                     {Population initialization};
...
\headerbox{5. Results}{name=sea,span=2,column=1,below=screen}{
    ...
    \begin{multicols}{2}
        \includegraphics[width=\linewidth]{selection_error}
...
\headerbox{6. Conclusions}{name=conclusion,column=1,below=sea,span=2,above=bottom}{
    ...
    As we have seen,
        feature selection is becoming very important in predictive analytics.
    Indeed,
        many data sets contain a large number of features,
        so we have to select the most useful ones.
