%!TEX root = ../slide.tex


% Current bad state, statistics with the problem
\subsection*{Context}
\begin{frame}{Context}{What is IoT ?} 
	\Figure{!htb}{.7}{context.png}{IoT platform}
	\Figure{!htb}{.7}{iotChallenges}{IoT challenges}
\end{frame}

% Why -> problem, This is due to the lack of
\subsection*{Problematic}
\begin{frame}{Problematic}{Where is the problem ?}
	\Columns{0.65}{0.35}{
		\Enumerate{
			\item How to Connect sensors to the best gateway?
			\Itemize{
				\item Decision and optimisation problem.
				\item Various network acces
				\item Various configuration of each network acces
				\item Lake of selection tools
			}
			\item How to connect sensors to this gateway with high Security level.
			\Itemize{
				\item Technical problem.
				\item Lake of selective tools
				\item How to select the \textbf{best} access point
			}
			\item How to extract knowledge from sensors data \supercite{thubert_6tisch_2015}.
			\Itemize{
				\item a
				\item Lake of selective tools
				\item How to select the \textbf{best} access point
			}
		}
	}{
		\Figure{!htb}{1}{stat_interoperability.png}{Key barriers in adopting the Industrial Internet \footfullcite{industrialinternetofthings_executive_}}
	}
\end{frame}

\begin{frame}{Problematic}{Where is the problem \cite{dimartino_internet_2018} ?}

\ac{BW}  \ac{SF} \ac{CR} \ac{Tx} \ac{RS} \ac{SNR} \ac{DR}  

\begin{table}[h!]
% \scriptsize
	\begin{tabular}{l|m{1mm}l|l|l}
	\textbf{Setting}& \multicolumn{2}{l|}{\textbf{Values}} 				    & \textbf{Rewards}		   & \textbf{Cost} 					    \\\hline
	\ac{BW}         & $125$ 	& \ding{224} $500 kHz$  								& \ac{DR}          		   & \ac{RS}, \blue{Range}. 			  \\\hline
	\ac{SF}         & $2^{6}$ 	& \ding{224} $2^{12}$ 									& \ac{RS}, \blue{Range}    & \ac{SNR}, longer packets, \ac{Tx}.    \\\hline
	\ac{CR}         & $4/5$ 	& \ding{224} $4/8$    								  	& Resilience 			   &  longer packets, \ac{Tx}. 				\\\hline
	\ac{Tx}         & $-4$ 		& \ding{224} $20 dBm$    								& \ac{SNR} 				   & \ac{Tx}  								\\\hline
	\end{tabular}
\caption{\label{tab:} \cite{cattani_experimental_2017}}
\end{table}

\end{frame}


% Future good state:  statistics without the problem}
%Who and what could we win if we resolve this probleme, why we didn't let it like it is
\subsection*{Motivation}
\begin{frame}{Motivations}{Why should we deal with such problems }
	\Columns{0.65}{0.35}{
		\Enumerate{
			\item 
			\Itemize{
				\item a
				\item Lake of selective tools
				\item How to select the \textbf{best} access point
			}
			\item QoS Analysis
			\Itemize{
				\item a
				\item Lake of selective tools
				\item How to select the \textbf{best} access point
			}
			\item Threats
			\Itemize{
				\item a
				\item Lake of selective tools
				\item How to select the \textbf{best} access point
			}
		}
	}{
		\Figure{!htb}{.7}{LPWAN.png}{Communication diversity}

	}
\end{frame}


% specifique, mesurables, atteignable, réalistic, time ?
% Reduce latency ...
% How can we validate 
\subsection*{Goal}
\begin{frame}{Goal}{Is it specific, measurable, achievable, réalistic, for 3 years ?}
	\Columns{0.65}{0.35}{
		\Itemize{ 
			\item 
			\Enumerate{
				\item Allow heterogeneous network to communicate
				\item QoS Analysis
				\item Threats
			}
			\item How to select the \textbf{best} access point
			\Enumerate{
				\item Allow heterogeneous network to communicate
				\item QoS Analysis
				\item Threats
			}
		}
	}{
		\Figure{!htb}{1}{wsn-IoT.png}{wsn-IoT}

	}
\end{frame}


% Why others didn't resolve the problem
\subsection*{Challenges}
\begin{frame}[bg]{Challenges}{Where is the difficulty ?}
	\Columns{0.65}{0.35}{
		\Enumerate{
			\item Challenge 1
			\Itemize{
				\item 6720 possible settings
				\item Lake of selective tools
				\item How to select the \textbf{best} configuration
			}
			\item Challenge 2
			\Itemize{
				\item a
				\item Lake of selective tools
				\item How to select the \textbf{best} access point
			}
			\item Challenge 3
			\Itemize{
				\item a
				\item Lake of selective tools
				\item How to select the \textbf{best} access point
			}
		}
	}{
		\Figure{!htb}{1}{mail.png}{tets}
	}
\end{frame}




\begin{frame}{Contributions}{Contributions}


\begin{itemize}
	\item Environnement
	\begin{itemize}
		\item Rural/Urban
		\item Static/Mobile
		\item 
	\end{itemize}
\end{itemize}

\begin{itemize}
	\item Senarios
	\begin{itemize}
		\item For each service (Smart building:Videos, Voice, Text. Smart trafic:Videos, Voice, Text)
		\item For each application protocol (MQTT, COAP, XMPP)
		\item For each network protocol (Start, Mesh)
		\item For each MAC protocol (LoraWan, Sigfox, ...)
		\item For each MAC configuration (SF, CR, BW, ...)
	\end{itemize}
\end{itemize}

\begin{itemize}
	\item Algorithms
	\begin{itemize}
		\item Input:
		\begin{itemize}
			\item Service QoS metrics requiremnts
			\item Network Transmission Parameters
			\item Network QoS metrics
		\end{itemize}


		\item Method:
		\begin{itemize}
			\item MADM
			\item Game
			\item Neural
		\end{itemize}

			\item Outputs:
		\begin{itemize}
			\item Ranked networks
		\end{itemize}
	\end{itemize}
\end{itemize}

\stamp{Simulation \& Real environnement}

\end{frame}
