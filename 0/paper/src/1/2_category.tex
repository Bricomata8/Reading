\section{Hardware and software platform for Internet of Things}
...
To qualify as an OS suitable for the IoT,
	...
	• Low Random-access memory (RAM) footprint • Low Read-only memory (ROM) footprint • Multi-tasking • Power management (PM) • Soft real-time These requirements are directly bound to the type of hardware designed for the IoT.
...
The overhead for each PHY packet is only 4+1+1 133 ∼ 4.5%;
	when
...
A full size IPv6 + UDP header is 40+8 bytes which is ∼ 38% of a IEEE 802.15.4
frame,
	but with the header compression this overhead can be reduced to 7 bytes,
	thus reducing the overhead to ∼ 5%,
...
Each frame can be transmitted 47bytes in 1Mbit/s = 376μs;
